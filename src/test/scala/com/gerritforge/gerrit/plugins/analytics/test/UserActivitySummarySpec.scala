...
import com.gerritforge.gerrit.plugins.analytics.UserActivitySummary
import com.gerritforge.gerrit.plugins.analytics.common.AggregationStrategy.EMAIL
import com.gerritforge.gerrit.plugins.analytics.common.{Statistics, TestUtils}
...
class UserActivitySummarySpec
  ...
  "numCommits" should "count only comments filtered by their merge status" in {
    ...
    val summary           = UserActivitySummary.apply(
      new Statistics(newProjectKey, commitsStatisticsNoCache)
    )(aggregatedCommits.head)
  ...
  def getRepoOwnedByPerson(
      email: String = author.getEmailAddress,
      repo: TestFileRepository = testFileRepository
  ): Repository = {
