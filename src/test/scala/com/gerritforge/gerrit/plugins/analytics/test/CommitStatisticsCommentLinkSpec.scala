...
import com.gerritforge.gerrit.plugins.analytics.IssueInfo
import com.gerritforge.gerrit.plugins.analytics.common.{CommitsStatistics, Statistics}
...
class CommitStatisticsCommentLinkSpec
    ...
    with TestCommitStatisticsNoCache
    with Matchers
    with Inside {
  ...
  class TestEnvironment(val repo: Repository = fileRepository) {
    ...
    testFileRepository.commitFile(
      ...
      """
        ...
        |[commentlink "link2"]
        |       match = "([Bb]ug:\\s+)(\\d+)"
        |       link = "http://trak.example.com/$2" """.stripMargin,
  ...
  it should "collect no commentslink if no matching" in new TestEnvironment {
    ...
    inside(stats.forCommits(nocomments)) { case List(s: CommitsStatistics) =>
      s.issues should have size 0
    }
  }
  it should "collect simple bugzilla comments" in new TestEnvironment {
    val simpleComment =
      testFileRepository.commitFile("file1.txt", "content2", message = "this solves bug #23")

    inside(stats.forCommits(simpleComment)) { case List(s: CommitsStatistics) =>
  ...
  it should "collect simple track link" in new TestEnvironment {
    val simpleTrackComment =
      testFileRepository.commitFile("file1.txt", "content3", message = "this solves Bug: 1234")

    inside(stats.forCommits(simpleTrackComment)) { case List(s: CommitsStatistics) =>
      s.issues should have size 1
  ...
  it should "collect multiple links" in new TestEnvironment {
    val multipleComments = testFileRepository.commitFile(
      "file1.txt",
      "content4",
      message = "this solves bug 12 and Bug: 23"
    )

    inside(stats.forCommits(multipleComments)) { case List(s: CommitsStatistics) =>
