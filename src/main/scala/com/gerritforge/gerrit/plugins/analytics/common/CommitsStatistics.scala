...
import com.gerritforge.gerrit.plugins.analytics.{CommitInfo, IssueInfo}
import org.eclipse.jgit.lib.ObjectId

/** Collects overall stats on a series of commits and provides some basic info on the included
  * commits
  *
  * @param addedLines
  *   sum of the number of line additions in the included commits
  * @param deletedLines
  *   sum of the number of line deletions in the included commits
  * @param isForMergeCommits
  *   true if the current instance is including stats for merge commits and false if calculated for
  *   NON merge commits. The current code is not generating stats objects for a mixture of merge and
  *   non-merge commits
  * @param isForBotLike
  *   true if the current instance is including BOT-like commits, false otherwise
  * @param commits
  *   list of commits the stats are calculated for
  */
...
class Statistics(projectNameKey: Project.NameKey, commitStatsCache: CommitsStatisticsCache) {
  ...
  /** Returns up to four different CommitsStatistics object grouping the stats into: Non Merge - Non
    * Bot Merge - Non Bot Non Merge - Bot Merge - Bot
    *
    * @param commits
    * @return
    */
