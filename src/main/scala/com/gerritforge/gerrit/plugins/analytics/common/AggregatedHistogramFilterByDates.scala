...
/** Commit filter that includes commits only on the specified interval starting from and to excluded
  */
class AggregatedHistogramFilterByDates(
  ...
  override def clone =
    new AggregatedHistogramFilterByDates(from, to, branchesExtractor, aggregationStrategy)
}
